...
class WordGuessingGameGUI:
    def __init__(self, master):
        ...
        self.label = tk.Label(master, text="Guess the word:", bg="lightblue", fg="black")
        self.label.pack()

        self.entry = tk.Entry(master, state='disabled', width=20, font=('Helvetica', 14), bg="lightgrey")
        self.entry.pack()

        self.remaining_guesses_label = tk.Label(master, text="Guesses left: 5", bg="lightblue", fg="black")
        self.remaining_guesses_label.pack()

        self.message_label = tk.Label(master, text="", bg="lightblue", fg="black")
        ...
        for char in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            button = tk.Button(self.buttons_frame, text=char, command=lambda c=char: self.guess_char(c), bg="black", fg = "white")
            button.pack(side=tk.LEFT)
            self.buttons.append(button)

        self.client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.HOST = '127.0.0.1'  # Change to the actual server IP address
        self.PORT = 5050  # Change to the actual server port
